angular.module('app').controller('PurchaseOrderDetailController', ['$scope', '$timeout', '$http', '$location', '$rootScope','BASE_URL', '$state', 'FileUploader', '$sce', 'localStorageService', '$stateParams',
function($scope, $timeout, $http, $location, $rootScope, BASE_URL, $state, FileUploader, $sce, localStorageService, $stateParams){
	...
	$scope.deletePurchaseOrderItem = function(key){
		$scope.purchase_order.purchase_order_items.splice(key, 1);
	}

	/* Copy Purchase Order */

    $scope.copyPurchaseOrder = function(){
		$rootScope.view_detail_purchase_order_id = $scope.purchase_order.id;
		$rootScope.is_copy_purchase_order = true;
