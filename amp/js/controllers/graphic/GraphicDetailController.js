angular.module('app').controller('GraphicDetailController', ['$scope', '$timeout', '$http', '$location', '$rootScope', 'BASE_URL', '$state', '$stateParams',
    function ($scope, $timeout, $http, $location, $rootScope, BASE_URL, $state, $stateParams) {
        ...
        $scope.getGraphicById = function () {
            $http.post(BASE_URL + '/graphic/getGraphicById', {id: $stateParams.id})
                    .success(function (data) {
                        if (data.success) {
        ...
        $scope.copyGraphic = function () {
            $rootScope.view_detail_graphic_id = $scope.graphic.id;
            $rootScope.is_copy_graphic = true;
            $state.go('graphic-create');
        }

        // Remove related graphics
        $scope.removeRelatedGraphic = function(removed_graphic, index){
          var information_post = {removed_graphic_id: removed_graphic.id, graphic_id: $scope.graphic.id};
          $http.post(BASE_URL + '/graphic/removeRelatedGraphic', information_post)
          .success(function (data) {
              if (data.success) {
                  swal('Graphic removed!', "", "success");
                  $scope.graphic.related_assets = data.related_assets;
                  $scope.graphic.related_graphics = data.related_graphics;
              }
              else {
                  swal({
        ...
        $scope.showListGraphic = function(){
            ...
            $scope.checkRelatedGraphic = function(checked_graphic){
              for(var i in $scope.graphic.related_graphics){
                var related_graphic = $scope.graphic.related_graphics[i];
            ...
            $scope.addRelatedGraphic = function(added_graphic){
              var information_post = {added_graphic_id: added_graphic.id, graphic_id: $scope.graphic.id};
              $http.post(BASE_URL + '/graphic/addRelatedGraphic', information_post)
              .success(function (data) {
                  if (data.success) {
        ...
        $scope.removeRelatedAsset = function(removed_asset, index){
          var information_post = {removed_asset_id: removed_asset.id, graphic_id: $scope.graphic.id};
          $http.post(BASE_URL + '/graphic/removeRelatedAsset', information_post)
          .success(function (data) {
              if (data.success) {
        ...
        $scope.showListAsset = function(){
            ...
            $scope.checkRelatedAsset = function(checked_asset){
              for(var i in $scope.graphic.related_assets){
                var related_asset = $scope.graphic.related_assets[i];
            ...
            $scope.addRelatedAsset = function(added_asset){
              var information_post = {added_asset_id: added_asset.id, graphic_id: $scope.graphic.id};
              $http.post(BASE_URL + '/graphic/addRelatedAsset', information_post)
              .success(function (data) {
                  if (data.success) {
