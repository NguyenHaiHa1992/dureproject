angular.module('app').controller('MaterialCreateController', ['$scope', '$timeout', '$http', '$location', '$rootScope','BASE_URL', '$state', '$filter', '$sce',
function($scope, $timeout, $http, $location, $rootScope, BASE_URL, $state, $filter, $sce){
	$scope.material= {};
	$scope.copy_material= {};
	...
	$scope.createInit= function(){
		...
		if(jQuery.type($rootScope.view_detail_material_id) !== "undefined" && $rootScope.view_detail_material_id!= ''){
			post_information= {id: $rootScope.view_detail_material_id};
			$rootScope.view_detail_material_id= undefined;
		}
		else{
			post_information= {};
		}
	...
	$scope.create= function(){
		...
		$http.post(BASE_URL + '/material/create', information_post)
	    .success(function(data) {
		    if(data.success) {
		    	swal('Material created!', "", "success");
		    	$scope.is_readonly = true;
		    	$state.go('material-list');
		    }
		    else{
		    	if(data.type== 'alert')
			    	swal({
