angular.module('app').controller('StoreListController', ['$scope', '$timeout', '$http', '$location', '$rootScope', 'BASE_URL', '$state', '$window',
	...
	$scope.root = $rootScope;
        ...
        $scope.getStores = function (post_information) {
            ...
            .success(function (data) {
                if (data.success) {
                    ...
                    $scope.start_store = data.start_store;
                    $scope.end_store = data.end_store;
                    $scope.pages = [];
                    ...
                    // $scope.stores.push(data.email_templates[i]);
        ...
        $scope.getStores(post_information);
        ...
        $scope.selectPage = function (page) {
            ...
            var post_information = {
                ...
                'sort_attribute': $scope.sort.attribute,
                'sort_type': $scope.sort.type,
                'id': $scope.search_store.id,
                'name': $scope.search_store.name,
                'tier_id': $scope.search_store.tier_id,
                'email': $scope.search_store.email,
                'created': $scope.search_store.created,
                'city': $scope.search_store.city,
            };
            $scope.getStores(post_information);
        };
        ...
        $scope.search = function () {
            post_information.tier_id = $scope.search_store.tier_id;
            post_information.name = $scope.search_store.name;
            post_information.email = $scope.search_store.email;
            post_information.city = $scope.search_store.city;
            post_information.signage_id = $scope.search_store.signage_id;
            post_information.fixture_id = $scope.search_store.fixture_id;
            
            $scope.copy_search_store.tier_id = $scope.search_store.tier_id;
            $scope.copy_search_store.name = $scope.search_store.name;
            $scope.copy_search_store.email = $scope.search_store.email;
            $scope.copy_search_store.city = $scope.search_store.city;
            $scope.copy_search_store.signage_id = $scope.search_store.signage_id;
            $scope.copy_search_store.fixture_id = $scope.search_store.fixture_id;
            $scope.getStores(post_information);
        }
        ...
        $scope.viewDetai = function (store_id) {
            $rootScope.view_detail_store_id = store_id;
        ...
        $scope.exportExcel = function(){
            var search_information = '&tier_id=' + $scope.copy_search_store.tier_id 
                    + '&name=' + $scope.copy_search_store.name 
                    + '&city=' + $scope.copy_search_store.city 
                    + '&email=' + $scope.copy_search_store.email
                    + '&signage_id=' + $scope.copy_search_store.signage_id
                    + '&fixture_id=' + $scope.copy_search_store.fixture_id;
        ...
        $scope.copyStore = function (id) {
//            $rootScope.view_detail_store_id = id;
//            $rootScope.is_copy_store = true;
//            $state.go('store-create');
            $http.post(BASE_URL + '/store/copy', {id: id})
            .success(function (data) {
                if (data.success) {
        ...
        $scope.exportPdfItem = function(storeId){
            $http.get(BASE_URL + '/store/exportPdf?id=' + storeId)
            .success(function (data) {
                if (data.success) {
                    swal({
        ...
        $scope.exportExcelItem = function(storeId){
            $http.get(BASE_URL + '/store/exportExcelItem?id=' + storeId)
            .success(function (data) {
                if (data.success) {
                    swal({
                        title: "Store Detail exported",
...
//                        }
	...
	$scope.importObject = function () {
            ...
            if (file) {
                swal({
                    title: "Store DB Import Option",
                    text: "<button class='email bg-green'><i class='fa fa-plus'></i> New store</button> \n\
                            <button class='download bg-blue'><i class='fa fa-edit'></i> Update store</button>\n\
                            <br><br><p>NOTE: Please use the sample file to list all stores you want to Add/Update with all info. \n\
        ...
        $scope.importProcess = function(file, option){
            ...
            if (ext == 'xls' || ext == 'xlsx' || ext == 'csv') {
                ...
                $http.post(BASE_URL + '/store/importObject', fd, {
        ...
        $scope.updateAfterImport = function(option, list_objects){
            if(option === "new"){
                $.each(list_objects, function (i, v) {
                    $scope.stores.unshift(v);
                });
            }
            else{
                $.each($scope.stores, function (si, sv) {
                    $.each(list_objects, function (di, dv) {
                        if(sv.store_number && sv.store_number.toString() === dv.store_number.toString()){
                            $scope.stores[si] = dv;
                        }
                    });
                });
