angular.module('app').controller('PartCreateController', ['$scope', '$timeout', '$http', '$location', '$rootScope','BASE_URL', '$state', '$sce',
function($scope, $timeout, $http, $location, $rootScope, BASE_URL, $state, $sce){
	...
	$scope.createInit= function(){
		...
		if(jQuery.type($rootScope.view_detail_part_id) !== "undefined" && $rootScope.view_detail_part_id!= ''){
			post_information= {id: $rootScope.view_detail_part_id};
			$rootScope.view_detail_part_id= undefined;
		}
		else{
			post_information= {};
		}
	    ...
	    .success(function(data) {
		    if(data.success) {
				...
				$scope.part_code= $scope.part.part_code;

				// If copy part
		    	if($rootScope.is_copy_part){
		    		$scope.part.id = undefined;
	...
	$scope.create= function(){
		...
		$http.post(BASE_URL + '/part/create', information_post)
	    .success(function(data) {
		    if(data.success) {
		    	$scope.part= data.part;
				$scope.part_copy = angular.copy($scope.part);
	...
	$scope.addHeatnumber = function(){
		if($scope.part.material.id == undefined){
	    	swal({
	    		title: 'Please select Material for Part before',
	    		text: '',
	    		type: 'error',
	    		html: true
	    	});
	...
	$scope.savePdfToUploadDocuments = function(){
		...
		if(check){
			var information_post = {
				'documents': $scope.email.documents, 
