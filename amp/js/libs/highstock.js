...
Chart.prototype = {
	...
	/**
	 * Render all signages for the chart
	 */
	render: function () {
		var chart = this,
			axes = chart.axes,
...
Point.prototype = {
	...
	destroy: function () {
		...
		// remove all events
		if (point.signage || point.dataLabel) { // removeEvent and destroyElements are performance expensive
			removeEvent(point);
	...
	destroyElements: function () {
		var point = this,
			props = ['signage', 'dataLabel', 'dataLabelUpper', 'group', 'connector', 'shadowGroup'],
			prop,
			i = 6;
		while (i--) {
			prop = props[i];
		...
		if (seriesMarkerOptions.enabled !== false || series._hasPointMarkers) {
			...
			while (i--) {
				...
				if (enabled && plotY !== UNDEFINED && !isNaN(plotY) && point.y !== null) {
					...
					if (signage) { // update
						signage[isInside ? 'show' : 'hide'](true) // Since the marker group isn't clipped, each individual marker must be toggled
							.animate(extend({
								x: plotX - radius,
								y: plotY - radius
							}, signage.symbolName ? { // don't apply to image symbols #507
								width: 2 * radius,
								height: 2 * radius
							} : {}));
					} else if (isInside && (radius > 0 || isImage)) {
						point.signage = signage = chart.renderer.symbol(
							symbol,
							plotX - radius,
							plotY - radius,
							2 * radius,
					...
					point.signage = signage.destroy(); // #1269
				}
			}
		}
				...
				point.redraw = function () {
					if (signage) {
						if (options && options.marker && options.marker.symbol) {
							point.signage = signage.destroy();
						} else {
							signage.attr(point.pointAttr[point.state || '']);
						}
					}
	...
	drawPoints: function () {
		...
		each(series.points, function (point) {
			...
			if (plotY !== UNDEFINED && !isNaN(plotY) && point.y !== null) {
				...
				if (signage) { // update
					stop(signage);
					signage.attr(borderAttr)[chart.pointCount < animationLimit ? 'animate' : 'attr'](merge(shapeArgs));

				} else {
					point.signage = signage = renderer[point.shapeType](shapeArgs)
						.attr(borderAttr)
						.attr(pointAttr)
						.add(series.group)
...
var PiePoint = extendClass(Point, {
	...
	setVisible: function (vis, force) {
		...
		if (vis !== point.visible || force) {
			...
			each(['signage', 'dataLabel', 'connector', 'shadowGroup'], function (key) {
				if (point[key]) {
	...
	slice: function (sliced, redraw, animation) {
		...
		point.signage.animate(translation);
...
var PieSeries = {
	...
	drawPoints: function () {
		...
		each(series.points, function (point) {
			...
			// draw the slice
			if (signage) {
				signage.animate(extend(shapeArgs, groupTranslation));
			} else {
				point.signage = signage = renderer[point.shapeType](shapeArgs)
...
var TrackerMixin = Highcharts.TrackerMixin = {
	...
	drawTrackerPoint: function () {
		...
		each(series.points, function (point) {
			if (point.signage) {
				point.signage.element.point = point;
			}
			if (point.dataLabel) {
...
/**
 * Add tracking event listener to the series group, so the point signages
...
extend(Point.prototype, {
	...
	setState: function (state, move) {
		var point = this,
			...
			stateMarkerSignage = series.stateMarkerSignage,
		...
		if (point.signage) {
			radius = markerOptions && point.signage.symbolName && pointAttr.r;
			point.signage.attr(merge(
				pointAttr,
			...
			if (stateMarkerSignage) {
				stateMarkerSignage.hide();
			}
		} else {
			// if a signage is not applied to each point in the normal state, create a shared
			// signage for the hover state
			if (state && markerStateOptions) {
				radius = markerStateOptions.radius;
				newSymbol = pointMarker.symbol || series.symbol;

				// If the point has another symbol than the previous one, throw away the
				// state marker signage and force a new one (#1459)
				if (stateMarkerSignage && stateMarkerSignage.currentSymbol !== newSymbol) {
					stateMarkerSignage = stateMarkerSignage.destroy();
				}

				// Add a new state marker signage
				if (!stateMarkerSignage) {
					if (newSymbol) {
						series.stateMarkerSignage = stateMarkerSignage = chart.renderer.symbol(
							newSymbol,
							plotX - radius,
							plotY - radius,
							2 * radius,
						...
						stateMarkerSignage.currentSymbol = newSymbol;
					}

				// Move the existing signage
				} else {
					stateMarkerSignage[move ? 'animate' : 'attr']({ // #1054
						x: plotX - radius,
						y: plotY - radius
					});
				}
			...
			if (stateMarkerSignage) {
				stateMarkerSignage[state && chart.isInsidePlot(plotX, plotY, chart.inverted) ? 'show' : 'hide'](); // #2450
			}
		}
		...
		if (xAxis && yAxis && (xAxis.options.breaks || yAxis.options.breaks)) {
			while (i--) {
				...
				if (xAxis.isInAnyBreak(point.x, true) || yAxis.isInAnyBreak(point.y, true)) {
					points.splice(i, 1);
					this.data[i].destroyElements(); // removes the signages for this point if they exist
				}
			}
		}
	...
	drawPoints: function () {
		...
		each(points, function (point) {
			if (point.plotY !== UNDEFINED) {
				...
				// create and/or update the signage
				if (signage) {
					signage
						.attr(pointAttr) // #3897
						.animate({ d: path });
				} else {
					point.signage = chart.renderer.path(path)
						.attr(pointAttr)
						.add(series.group);
				}
...
seriesTypes.flags = extendClass(seriesTypes.column, {
		...
		while (i--) {
			...
			if (plotY !== UNDEFINED && plotX >= 0 && !outsideRight) {
				...
				if (signage) { // update
					...
					signage = point.signage = renderer.label(
	...
	drawTracker: function () {
		...
		each(points, function (point) {
			var signage = point.signage;
			if (signage) {
				addEvent(signage.element, 'mouseover', function () {
					...
					each(points, function (otherPoint) {
						if (otherPoint !== point && otherPoint.raised && otherPoint.signage) {
							otherPoint.signage.attr({
								y: otherPoint._y
							});
