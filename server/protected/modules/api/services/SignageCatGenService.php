 ...
 * @author hanguyenhai
 */
class SignageCatGenService extends iPhoenixService{
    public static function getAll($data = []){
        $result= array();
        $criteria = new CDbCriteria();
        $criteria->condition = "status = 1";
        $criteria->order = "name ASC";
        $categories= SignageGeneralCategory::model()->findAll($criteria);
    ...
    public static function update($data){
        $result = array();
        if (isset($data['id'])) {
            $categoryFind = SignageGeneralCategory::model()->findByPk((int) $data['id']);
            if(!$categoryFind){
            ...
            // Create new category
            $new_category = new SignageGeneralCategory();
    ...
    public static function removeCategory($data) {
        $result = array();
        $category = SignageGeneralCategory::model()->findByPk((int) $data['id']);
        if ($category->delete()) {
            $result['success'] = true;
