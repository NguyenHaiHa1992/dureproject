...
class Shape extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('status, image_id', 'numerical', 'integerOnly'=>true),
	...
	protected function beforeDelete()
	{
        ...
        $criteria = new CDbCriteria();
        $criteria->compare('shape_id', $this->id);
        $material = Material::model()->find($criteria);

        if(isset($material)){
			$this->addError('name', 'This Shape can not be deleted because there are some existed items related to it');
			return false;
        }
        else{
